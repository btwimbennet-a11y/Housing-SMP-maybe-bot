...
// Render requires web services to bind to a port within 60s.
...
server.listen(PORT, () => {
  console.log(`Web server listening on port ${PORT} (required for Render)`);
});

// Configuration - Edit these values for your server
const config = {
  server: {
    host: 'eu1.freegamehost.xyz', // Change to your server IP
    port: 25054,
    version: '1.21.11' // Change to your server version
  },
  bot: {
    username: 'alwaysonline', // Change to your desired bot name
    auth: 'offline', // 'offline', 'microsoft', or 'mojang'
    password: '', // Minecraft account password (if using premium auth)
    authmePassword: 'ApplePie22' // AuthMe password for /register and /login
  },
  serverCommands: {
    enabled: true,
